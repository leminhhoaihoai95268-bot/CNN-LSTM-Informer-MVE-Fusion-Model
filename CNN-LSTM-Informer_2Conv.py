# -*- coding: utf-8 -*-
"""
Two-layer CNN-LSTM-Informer + Three-stage MVE + Split Conformal Calibration
5 contiguous folds x 5 random seeds

Experiment design
-----------------
1) The full sequence is split IN ORDER into five contiguous folds (~20% each).
2) Each fold is used once as the test fold; the other four folds are training-side data.
3) Inside EACH of the four non-test folds, the first 90% is used for proper model training.
   The last 10% is shared by validation and conformal calibration:
      - validation role: ONLY record per-epoch validation loss;
      - calibration role: compute split-conformal calibration scores and q_hat after training.
   Validation loss is NOT used for early stopping, model selection, hyperparameter tuning,
   learning-rate adjustment, checkpoint selection, or any other training decision.
4) Scalers are fitted ONLY on the proper model-training rows. The shared VAL/CAL segment
   and the test fold do not participate in scaler fitting.
5) MVE uses three stages:
      Stage 1: mean learning (MSE)
      Stage 2: variance learning (Gaussian NLL with Stage-1 residuals)
      Stage 3: joint MVE optimization (Gaussian NLL)
6) After Stage 3, standardized calibration scores are computed on the shared VAL/CAL segment:
      s_i = |y_i - mu_i| / (sigma_i + eps)
   and the finite-sample split-conformal 95% quantile q_hat is obtained.
7) Only the final conformal-calibrated interval is reported:
      [mu - q_hat*sigma, mu + q_hat*sigma]
   No uncalibrated Gaussian 1.96-sigma comparison interval is output.
   Interval metrics follow the manuscript notation: PICR, PIAW, NPIAW and
      CWC = (1 - NPIAW) * exp[-eta * (PICR - 0.95)^2], eta = 30.
8) Each test fold is repeated with seeds 42,43,44,45,46 => 25 independent runs.

Target-leakage control
----------------------
The prediction target is column I. Model inputs contain ONLY columns A-H; no true I-stage
value is included in any input window. Feature scaling uses A-H only, while target scaling
is fitted separately on I using proper-training rows only.

Important time-series detail
----------------------------
Windows are constructed within each original contiguous fold. Training windows never cross
fold boundaries. Shared VAL/CAL targets are restricted to the held-out last 10% of each
training-side fold; their look-back context may include immediately preceding proper-training
rows from the SAME original fold, which is valid for sequential one-step-ahead prediction.
"""
...
def split_train_valcal_inside_fold(fold_idx, proper_train_ratio=0.90):
    """
    Preserve chronology inside one original fold:
      first 90% -> proper training rows
      last 10%  -> shared validation/conformal-calibration rows

    The last 10% has two roles but is the SAME set of samples:
      1) validation_data during fit, only to record val_loss;
      2) split-conformal calibration after all training is finished.
    """
    n = len(fold_idx)
    split_pos = int(np.floor(n * proper_train_ratio))

    # Keep both parts usable. The shared VAL/CAL targets may use preceding
    # training context from the SAME original fold.
...
def make_windows_by_target_positions(
    ...
    """
    Build one-step windows inside ONE original contiguous fold.

    target_positions are local positions inside this fold. Each target may use the preceding
    look_back rows from the SAME fold only. This prevents artificial adjacency across folds.
    """
    X, y, global_targets = [], [], []
...
def prepare_fold_data(raw_features, raw_target, fold_indices, test_fold_id):
    """
    For the current CV fold:
      - one original ~20% fold is test;
      - in each of the other four folds, first 90% is proper training;
      - the last 10% is the SAME shared validation/calibration segment;
      - scalers are fitted only on proper-training rows;
      - windows never cross original fold boundaries.
    """
    ...
    # Strict preprocessing: the shared VAL/CAL segment and test rows do NOT
    # participate in scaler fitting.
    ...
    for fid in train_fold_ids:
        ...
        # Shared VAL/CAL targets: split_pos ... end of this same original fold.
        # During model.fit this set ONLY reports val_loss; after training the
        # identical samples are used to estimate conformal q_hat.
    ...
    # Validation and calibration deliberately point to the SAME held-out samples.
    X_val = X_valcal
    y_val = y_valcal
    ...
    # Test windows stay strictly inside the selected ~20% test fold.
...
def conformal_multiplier(y_true_scaled, mu_scaled, std_scaled, alpha=0.05):
    """
    Standardized split-conformal score:
        score_i = |y_i - mu_i| / (sigma_i + eps)

    Finite-sample quantile index:
        k = ceil((n + 1) * (1 - alpha))
    q_hat is the k-th order statistic (capped at n).
    """
...
def interval_metrics(y_true, lower, upper, nominal_coverage=0.95, eta=30.0):
    """
    Prediction-interval metrics using the definitions adopted in the manuscript:

        PICR = coverage rate of the prediction interval
        PIAW = mean prediction-interval width
        R    = max(y_true) - min(y_true)
        NPIAW = PIAW / R
        CWC  = (1 - PIAW / R) * exp[-eta * (PICR - nominal_coverage)^2]

    Under this definition, a larger CWC is better: it rewards a narrow interval
    while penalizing deviation of PICR from the desired coverage rate (0.95 here).
    """
    y_true = np.asarray(y_true).reshape(-1)
    lower = np.asarray(lower).reshape(-1)
    upper = np.asarray(upper).reshape(-1)

    # PICR: Prediction Interval Coverage Rate
    picr = np.mean((y_true >= lower) & (y_true <= upper))

    # PIAW: Prediction Interval Average Width
    piaw = np.mean(upper - lower)

    # R: range of the true values in the evaluated test set
...
def save_three_stage_loss(
    ...
    """Save training/validation loss curves and raw data for all three MVE stages.

    Validation loss is diagnostic only and never feeds back into training decisions.
    """
